...
ml <- model
pred <- predict(ml,ts_price[2:31]) #label 체크하기

mx <- matrix(rep(0,len=38),ncol=38,nrow=1) 
mx <- as.data.frame(mx)
rownames(mx) <- c("무")
colnames(mx) <- date2+7*wk
mx[1,] <- pred
View(mx)
...
NM <- c() #nmae값 담기
NM <- c(NM,nm(mx[n,],real_price[,2*n+1]))
